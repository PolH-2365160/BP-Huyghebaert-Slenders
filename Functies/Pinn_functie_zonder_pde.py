...
def compute_losses(model: nn.Sequential, x_data: torch.Tensor, t_data: torch.Tensor, target_data: torch.Tensor) -> torch.Tensor:
...
def Pinn_zonder_pde(basis_pad: Path, seed: int = 42, naam_model: str = '', L: int = 20, T: int = 15, r: int = 1, D: int = 1, EPOCHS: int = 8000, Nx: int = 20000, Nt: int = 3200, 
                                    aantal_trainingspunten: int = 5000, lam_data: float = 1.0, plot_total_loss_name: str = '', plot_comparison_loss_name: str = '',
    ...
    Args:
        ...
        ruis (float, optional): De standaarddeviatie van de toegevoegde ruis als fractie van de standaarddeviatie van de data. Defaults to 0.
    ...
    loss_hist_tot, loss_hist_data = [] , []#loss_hist_tot, loss_hist_pde, loss_hist_init, loss_hist_bdy, loss_hist_data = [], [], [], [], []
    K_history = []
    ...
    for epoch in range(EPOCHS):
        ...
        # Bereken de component van de loss met behulp van de compute_losses functie
        loss_data = compute_losses(
            model, x_train_data, t_train_data, trainings_data
        )
        
        # Bereken de totale loss
        total_loss = lam_data * loss_data
        ...
        if epoch % 100 == 0:
            ...
            if epoch > 0:
                ...
                print(f"Epoch {epoch}/{EPOCHS} | Tot Loss: {total_loss.item():.2e} | Rest: {minutes}m {seconds}s")
            else:
                print(f"Epoch {epoch}/{EPOCHS} | Tot Loss: {total_loss.item():.2e}")
    ...
    if plot_comparison_loss_name != '':
        ...
        ax.set_xlabel("Trainingsiteratie", fontsize=14)
        ax.set_ylabel("Loss", fontsize=14)
        ax.set_title("Vergelijking Training Loss", fontsize=16)
        ax.legend(
        fontsize=14,
    ...
    if 'Leersnelheid' in returns:
        ...
        return K_history, K_inf_pred, alpha_pred
    if 'losses_history' in returns:
        return loss_hist_tot, loss_hist_data
    if 'model' in returns:
        return model
